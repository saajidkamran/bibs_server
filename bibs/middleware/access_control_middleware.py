# middleware/access_control_middleware.py
...
class AccessControlMiddleware:
    ...
    def __call__(self, request):
        if request.path in ["/api/login/", "/api/token/refresh/", "/api/token/verify/"]:
        ...
        try:
            ...
            if not auth_header:
                return JsonResponse(
                    {"error": "Authorization header missing."}, status=403
                )
            ...
            print("nEMPCODE>>", nEMPCODE)

            # Step 4: Attach employee code to request
            request.nEmployeeCode = nEMPCODE  # <-- Set here 🔥

            # Step 5: Access control (your existing code)
            ...
            if method in ["PUT", "PATCH"] and not (
                access_right.edit or access_right.update
            ):
